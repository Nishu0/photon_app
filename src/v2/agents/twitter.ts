  ...
  const server = createSdkMcpServer({
    ...
    tools: [
      tool(
        ...
        async ({ handle, filter }) => {
          const user = await deps.twitter.getUserByUsername(normalizeHandle(handle));
          if (!user) {
            return {
              content: [{ type: "text", text: JSON.stringify({ ok: false, error: `no user @${handle}` }) }]
            };
          }
          upsertWatch(deps.store, { handle: user.userName, user_id: user.id, filter });
          return {
            content: [
              {
                type: "text",
      ...
      tool(
        ...
        async ({ handle }) => {
          const removed = deleteWatch(deps.store, handle);
          return {
            content: [
              { type: "text", text: JSON.stringify({ ok: removed, handle: normalizeHandle(handle) }) }
            ]
          };
        }
      ),
      tool("x_list_watches", "List every X handle kodama is currently watching.", {}, async () => {
      ...
      tool(
        ...
        async ({ handle, hours }) => {
          ...
          const tweets = await deps.twitter.recentFromUser(normalizeHandle(handle), sinceUnix);
          return {
            content: [
              {
                type: "text",
